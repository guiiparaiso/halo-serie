...
(function konamiGame() {
  ...
  (function secretLogoTap() {
    ...
    // touchend é o evento mais confiável para contar taps no Chrome Android
    // — não tem delay de 300ms, não conflita com gestures do browser
  ...
  function showRotateScreen(onDone) {
    ...
    // CSS precisa existir antes de criar o elemento
    injectStyles();
    ...
    document.getElementById('sg-rotate-skip').addEventListener('click', function() {
      el.remove();
      window.removeEventListener('resize', checkOrientation);
      onDone();
    });
  ...
  function _openMenuReal() {
    ...
    menu.innerHTML = `
      ...
      <div style="font-size:0.55rem;letter-spacing:2px;color:rgba(0,200,255,0.35);margin-top:0.5rem;font-family:'Rajdhani',sans-serif;line-height:1.9;" id="sg-controls-hint">
    ...
    const hint = menu.querySelector('#sg-controls-hint');
    if (hint) {
      const isTouchDevice = window.matchMedia('(hover: none), (pointer: coarse)').matches;
      hint.textContent = isTouchDevice
        ? 'JOYSTICK · MOVER  ·  🔴 TIRO  ·  ⚡ DASH  ·  PODERES À DIREITA  ·  ESC · SAIR'
        : '← → / A D · MOVER  ·  SHIFT · DASH  ·  ESPAÇO · SUPER TIRO  ·  1-4 · PODERES  ·  ESC · SAIR';
    }
  ...
  function startGame(mode) {
    ...
    let joystickDx = 0;
    let touchShooting = false, touchShootInterval = null;
    // Declaradas aqui para que endGame possa removê-las
    let onTouchMove = function(){};
    let onTouchEnd  = function(){};

    if (isTouchDevice) {
      // Cria container de controles touch
      touchControls = document.createElement('div');
      touchControls.id = 'sg-touch-controls';
      touchControls.className = 'visible';

      // Joystick base + knob
      const jZone = document.createElement('div'); jZone.id = 'sg-joystick-zone';
      const jBase = document.createElement('div'); jBase.id = 'sg-joystick-base';
      const jKnob = document.createElement('div'); jKnob.id = 'sg-joystick-knob';
      jZone.appendChild(jBase); jZone.appendChild(jKnob);
      touchControls.appendChild(jZone);

      // Botão de tiro
      const btnShoot = document.createElement('div'); btnShoot.id = 'sg-btn-shoot';
      btnShoot.innerHTML = '🔴<span>TIRO</span>';
      touchControls.appendChild(btnShoot);

      // Botão dash
      const btnDash = document.createElement('div'); btnDash.id = 'sg-btn-dash';
      btnDash.innerHTML = '⚡<span>DASH</span>';
      touchControls.appendChild(btnDash);

      // Botões de poderes touch (coluna lateral)
      const touchPowersEl = document.createElement('div'); touchPowersEl.id = 'sg-touch-powers';
      const touchPowerSlots = POWERS.map((p, i) => {
        const slot = document.createElement('div');
        slot.className = 'sg-touch-power ready';
        slot.innerHTML = `${p.icon}<div class="sg-cd-fill" id="sg-tcd-${i}" style="height:0%"></div>`;
        slot.addEventListener('touchstart', e => { e.stopPropagation(); e.preventDefault(); activatePower(i); }, {passive:false});
        touchPowersEl.appendChild(slot);
        return slot;
      });
      touchControls.appendChild(touchPowersEl);

      wrap.appendChild(touchControls);
      ...
      onTouchMove = function(e) {
        ...
        for (let t of e.changedTouches) {
          ...
          jKnob.style.transform = `translate(calc(-50% + ${knobX}px), calc(-50% + ${knobY}px))`;
          // Guarda normX para o update loop aplicar com slowFactor
          joystickDx = dx / JRADIUS; // -1 a 1
        }
        e.preventDefault();
      }
      ...
      // ── Botão tiro: segurar para atirar continuamente ──
      btnShoot.addEventListener('touchstart', e => {
        e.preventDefault();
        btnShoot.classList.add('active');
        keys[' '] = false; // garante que não está no modo charge
        touchShooting = true;
      }, {passive:false});
      ...
      btnDash.addEventListener('touchstart', e => {
        ...
        if (P.dashCharges > 0 && !P.dashing) {
          // Dash na direção que o joystick aponta (ou para a direita por padrão)
          const dir = joystickActive
            ? (Math.cos(Math.atan2(
                joystickOriginY - (jKnob.getBoundingClientRect().top + 24),
                jKnob.getBoundingClientRect().left + 24 - joystickOriginX
              )) > 0 ? 1 : -1)
            : (P.x < W()/2 ? 1 : -1);
          P.dashing = true; P.dashVx = dir * 18; P.dashTimer = 180; P.inv = 400;
          P.dashCharges = Math.max(0, P.dashCharges - 1);
        ...
        setTimeout(() => btnDash.classList.remove('active'), 200);
      }, {passive:false});

      // ── Atualiza visual dos power slots touch (sincroniza com powerCooldowns) ──
      const origUpdatePowers = () => {
        POWERS.forEach((_, i) => {
          const slot = touchPowerSlots[i];
          const fill = document.getElementById(`sg-tcd-${i}`);
          if (powerCooldowns[i] > 0) {
            slot.className = 'sg-touch-power cooldown';
      ...
      // Sobrescreve o final do update para incluir sync touch powers
      const origUpdate = update;
      // Hook: chama sync após cada frame (via sobrescrita do loop)
      const origLoop = loop; // referenciado abaixo
      // Usamos um flag para rodar junto no loop principal
      wrap._syncTouchPowers = origUpdatePowers;
    }
    ...
    function update(dt) {
      ...
      if(P.dashing){
        ...
        // Joystick touch (com slowFactor aplicado)
        if(isTouchDevice && joystickActive && Math.abs(joystickDx) > 0.15) {
          const jspd = P.spd * Math.min(Math.abs(joystickDx), 1) * 1.5 * sf;
          P.x += joystickDx > 0 ? jspd : -jspd;
        }
      }
      ...
      // Auto shoot — teclado OU botão touch
      if(!P.charging){
        shootTimer+=dt;
        const interval=powerActive[0]?90:160;
        if(shootTimer>=interval){
          shootTimer=0;
          if(!isTouchDevice || touchShooting) {
      ...
      if(mode==='classic'||mode==='shield'){
        if(enemies.length===0&&!bossAlive){
          if(wave%5===0){ spawnBoss(bossPhase); wave++; }
          else { wave++; setTimeout(()=>{ showAnnounce(`WAVE ${wave}`,'#00ffe7',1500); spawnWave(wave); },1400); }
        }
      ...
      dashInd.textContent = isTouchDevice ? `DASH ⚡ ${dots}` : `DASH [SHIFT] ${dots}`;
      dashInd.style.color=P.dashCharges>0?'rgba(0,200,255,0.55)':'rgba(255,100,0,0.5)';

      // Sync touch power slots
      if (wrap._syncTouchPowers) wrap._syncTouchPowers();
    }
    ...
    function endGame(won){
      ...
      // Remove listeners de touch para não travar o scroll após fechar o jogo
      if (isTouchDevice) {
...
} // fim do _initHaloScript

// Funciona independente do timing: se o DOM já está pronto, roda direto
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', _initHaloScript);
} else {
  _initHaloScript();
}
